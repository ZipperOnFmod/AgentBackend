...
app.post("/account/api/oauth/token", async (req, res) => {
    ...
    try {
        ...
        return error.createError("errors.com.epicgames.common.oauth.invalid_client", "It appears that your Authorization header may be invalid or not present, please verify that you are sending the correct headers.", [], 1011, "invalid_client", 400, res);
    }
    switch (req.body.grant_type) {
        ...
        case "password":
            if (!req.body.username || !req.body.password)
                return error.createError("errors.com.epicgames.common.oauth.invalid_request", "Username/password is required.", [], 1013, "invalid_request", 400, res);
            ...
            let err = () => error.createError("errors.com.epicgames.account.invalid_account_credentials", "You Have Entered The Wrong Login Code, Check Your DMs With Agent Bot And Try Again.", [], 18031, "invalid_grant", 400, res);
            if (!req.user)
                return err();
            else {
                if (!rebootAccount) {
            ...
            if (req.user.mfa) {
                ...
                const embed = new EmbedBuilder()
                    .setColor('#A020F0')
                    .setTitle('MFA Login Alert')
                    .setDescription("A new login has been detected. If this was you click Yes.")
                    .addFields({ name: 'Account name', value: req.user.username, inline: true }, { name: 'IP Address', value: req.ip, inline: true })
                    .setTimestamp()
                    .setFooter({
                    text: 'Agent',
                    iconURL: 'http://projectAgent.xyz/assets/img/Logo.png'
                });
                const yesButton = {
                    type: 2,
                    style: 3,
                ...
                collector.on('collect', async (i) => {
                    ...
                    switch (i.customId) {
                        ...
                        case 'no':
                            await kv.set(req.user.discordId, "false");
                            error.createError("errors.com.epicgames.account.oauth.user_denied", "User denied the request.", [], 18058, "access_denied", 400, res);
        ...
        case "refresh_token":
            if (!req.body.refresh_token)
                return error.createError("errors.com.epicgames.common.oauth.invalid_request", "Refresh token is required.", [], 1013, "invalid_request", 400, res);
            ...
            try {
                ...
                error.createError("errors.com.epicgames.account.auth_token.invalid_refresh_token", `Sorry the refresh token '${refresh_token}' is invalid`, [refresh_token], 18036, "invalid_grant", 400, res);
                return;
            }
        ...
        case "exchange_code":
            if (!req.body.exchange_code)
                return error.createError("errors.com.epicgames.common.oauth.invalid_request", "Exchange code is required.", [], 1013, "invalid_request", 400, res);
            ...
            if (index == -1)
                return error.createError("errors.com.epicgames.account.oauth.exchange_code_not_found", "Sorry the exchange code you supplied was not found. It is possible that it was no longer valid", [], 18057, "invalid_grant", 400, res);
            global.exchangeCodes.splice(index, 1);
            req.user = await User.findOne({ accountId: exchange.accountId }).lean();
            break;
        default:
            error.createError("errors.com.epicgames.common.oauth.unsupported_grant_type", `Unsupported grant type: ${req.body.grant_type}`, [], 1016, "unsupported_grant_type", 400, res);
            return;
    }
    if (req.user.banned === undefined)
        return error.createError("errors.com.epicgames.account.oauth.account_not_found", "Sorry the account you are trying to login to does not exist", [], 18056, "invalid_grant", 400, res);
    if (req.user.banned)
        return error.createError("errors.com.epicgames.account.account_not_active", "You have been permanently banned from Fortnite.", [], -1, undefined, 400, res);
