...
const getCloudFile = async (objectName) => {
    const params = {
...
const listCloudFiles = async (prefix) => {
    const objectsList = [];
...
const createCloudStorageFolder = async (uid) => {
...
app.get("/fortnite/api/cloudstorage/system", async (req, res) => {
    if (Safety.env.USE_S3) {
        try {
            ...
            const localFiles = fs.readdirSync(path.join(__dirname, "../../", "hotfixes"));
            for (const file of localFiles) {
                const key = `CloudStorage/${uid}/${file}`;
                const object = folderObjects.find(obj => obj.Key === key);
                if (!object) {
                    const fileData = fs.readFileSync(path.join(__dirname, "../../", "Hotfixes", file));
                    const params = {
                        Bucket: Safety.env.S3_BUCKET_NAME || "BucketNotSet",
                        Key: key,
                        Body: fileData,
        ...
        const dir = path.join(__dirname, "../../", "hotfixes");
        let CloudFiles = [];
...
app.get("/fortnite/api/cloudstorage/system/:file", async (req, res) => {
    if (Safety.env.USE_S3) {
        ...
        const file = path.join(__dirname, "../../", "Hotfixes", req.params.file);
        log.debug(`File: ${file}`);
...
app.get("/api/v1/fortnite-br/surfaces/*/target", async (req, res) => {
