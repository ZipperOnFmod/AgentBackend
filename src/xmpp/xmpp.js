...
app.get("/clients", (req, res) => {
    ...
    let amount = global.Clients.length.toString(); // Convert number to string
    res.send(amount); // Send just the number
});
wss.on('listening', () => {
    logger.xmpp(`Listening On [${port}]`);
});
        ...
        switch (msg.root.name) {
            ...
            case "auth":
                ...
                if (decodedBase64 && accountId && displayName && token && decodedBase64.length == 3) {
                    Authenticated = true;
                    console.log(`[${purple}SUCCESSFUL LOGIN:${reset}] ${displayName} [${green}${timeString}${reset}]`);
                ...
                switch (msg.root.attributes.id) {
                    case "_xmpp_bind1":
                        if (resource || !accountId)
                            return;
                        if (!msg.root.children.find(i => i.name == "bind"))
                            return;
            ...
            case "message":
                ...
                switch (msg.root.attributes.type) {
                    ...
                    case "groupchat":
                        ...
                        if (!MUC)
                            return;
                        if (!MUC.members.find(i => i.accountId == accountId))
                            return;
                        return;
                }
                if (isJSON(body)) {
                    ...
                    if (Array.isArray(bodyJSON))
                        return;
                    if (typeof bodyJSON.type != "string")
                        return;
                    if (!msg.root.attributes.to)
                        return;
                    if (!msg.root.attributes.id)
                        return;
            ...
            case "presence":
                    ...
                    case "unavailable":
                        ...
                        if (msg.root.attributes.to.endsWith(`@muc.${domain}`) || msg.root.attributes.to.split("/")[0].endsWith(`@muc.${domain}`)) {
                            ...
                            if (memberIndex != -1) {
                                ...
                                //TODO: Fix type error without TSIgnore
                                //@ts-ignore
                ...
                if (!findStatus || !findStatus.content)
                    return;
                if (!isJSON(findStatus.content))
                    return;
                if (Array.isArray(JSON.parse(findStatus.content)))
                    return;
...
function RemoveClient(ws, joinedMUCs) {
    ...
    if (partyId && typeof partyId == "string") {
        global.Clients.forEach(ClientData => {
            if (client.accountId == ClientData.accountId)
                return;
    ...
    console.log(`[${purple}CLIENT LOGGED-OUT:${reset}] ${client.displayName} [${green}${timeString}${reset}]`);
}
async function getPresenceFromFriends(ws, accountId, jid) {
    ...
    accepted.forEach(friend => {
        ...
        if (ClientData.lastPresenceUpdate.away)
            xml = xml.element("show", "away").up().element("status", ClientData.lastPresenceUpdate.status).up();
        else
