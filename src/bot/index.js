import { Client, Partials, Collection, Events, GatewayIntentBits, ActivityType } from 'discord.js';
...
const foldersPath = path.join(basePath, 'src', 'bot', 'commands');
...
client.once(Events.ClientReady, async () => {
    logger.bot(`Connected To ${client.user?.tag}!`);
});
...
client.on(Events.GuildBanAdd, async (ban) => {
    if (!Safety.env.ENABLE_CROSS_BANS)
        return;
