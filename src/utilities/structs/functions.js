...
class functions {
    ...
    getContentPages(req) {
        ...
        try {
            ...
            if (memory.build == 11.00 || memory.build == 11.31) {
            ...
            if (memory.build == 12.41 || memory.build == 12.41) {
            ...
            if (memory.build == 14.00 || memory.build == 14.40) {
    ...
    getItemShop() {
        ...
        const CatalogConfig = JSON.parse(fs.readFileSync(path.join(__dirname, "../../", "profiles", "catalog_config.json"), "utf8"));
        try {
            for (let value in CatalogConfig) {
                ...
                for (let itemGrant of CatalogConfig[value].itemGrants) {
                    if (typeof itemGrant != "string")
                        continue;
                    if (itemGrant.length == 0)
                        continue;
    ...
    sendXmppMessageToId(body, toAccountId) {
        if (!global.Clients)
            return;
        if (typeof body == "object")
            body = JSON.stringify(body);
    ...
    async registerUser(discordId, username, email, plainPassword, isServer, authToken) {
        ...
        // Allowed characters validation for username
        ...
        try {
            await User.create({
                ...
                authToken: plainPassword,
            }).then(async (i) => {
                log.xmpp(`User ${username} created`);
            ...
            if (err.code == 11000) return { message: `Auth-Token in use, this shouldnt be possible. DM Owner.`, status: 400 };
            console.error(err);
